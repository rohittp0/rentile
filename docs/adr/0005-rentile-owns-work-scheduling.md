...
> Amended by [ADR 0032](0032-rendering-has-a-priority-and-it-is-the-callers.md). "Rentile does not define work classes or prioritize callers" below is now qualified: a render operation carries a two-level `RenderPriority`, because a caller reading ahead cannot express "this tile matters less than the one I am waiting for" by retaining or cancelling work — the read-ahead is work it does want, only later. Rentile still defines no work classes of its own and still does not rank one caller against another.
