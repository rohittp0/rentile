...
- **Layout properties**: `text-font`, `text-size`, `text-anchor`, `text-offset`, `text-justify`, `text-max-width`, `text-letter-spacing`, `text-line-height`, `text-transform`, `text-padding`, `text-allow-overlap`, `text-ignore-placement`, `symbol-sort-key`, and paint's `text-color`, `text-opacity` and `text-halo-*`.

Anything outside that set keeps Rentile's established behaviour: exclude the layer with a diagnostic rather than guess.
